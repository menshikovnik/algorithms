...
public class SimpleBinaryTree<T> implements BinaryTree<T> {
    ...
    public SimpleTreeNode getTreeRoot(){
        return root;
    }
    ...
    public void countingLeftAndRightForEachNode(SimpleTreeNode node){
        if (node.left != null){
            node.left.countLeft = node.countLeft + 1;
            node.left.countRight = node.countRight;
            countingLeftAndRightForEachNode(node.left);
        }
        if (node.right != null){
            node.right.countRight = node.countRight + 1;
            node.right.countLeft = node.countLeft;
            countingLeftAndRightForEachNode(node.right);
        }

    }
    public int resultCountLeft = 0;
    public int resultCountRight = 0;
    public void displayingCountLeftAndCountRightValues(SimpleTreeNode root){
        if (root.left != null){
            if (root.left.countLeft > root.left.countRight){
                resultCountLeft++;
            } else if(root.left.countLeft < root.left.countRight){
                resultCountRight++;
            }
            displayingCountLeftAndCountRightValues(root.left);
        }
        if (root.right != null){
            if (root.right.countLeft > root.right.countRight){
                resultCountLeft++;
            } else if(root.right.countLeft < root.right.countRight){
                resultCountRight++;
            }
            displayingCountLeftAndCountRightValues(root.right);
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        tree.countingLeftAndRightForEachNode(tree.root);
        tree.displayingCountLeftAndCountRightValues(tree.root);
        System.out.println("Чаще влево, чем вправо" + " " + tree.resultCountLeft);
        System.out.println("Чаще вправо, чем влево" + " " + tree.resultCountRight);
    }
}
